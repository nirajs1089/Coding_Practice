...
zombieCluster has the following parameter(s):
    ...
    zombies[zombies[0],...zombies[n-1]]:  an array of strings of binary digits z[i] representing connectedness of zombies
...
The next n lines each contain a binary string of length n describing a row in the matrix, zombies[i] where 0 ≤ i < n.
...
We have n = 4 zombies numbered z[0] through z[3]. There are 2 pairs of zombies who directly know each another: (z[0], z[1]) and (z[1], z[2]). Because of zombiism's transitive property, the set of zombies {z[0], z[1], z[2]} is considered to be a single zombie cluster. The remaining zombie, z[3], doesn't know any other zombies and is considered to be his own, separate zombie cluster ({z[3]}). This gives us a total of 2 zombie clusters.
...
No zombie knows who any other zombie is, so they each form their own zombie cluster: {z[0]}, {z[1]}, {z[2]}, {z[3]}, and {z[4]}. This means we have 5 zombie clusters, so we print 5 on a new line.
